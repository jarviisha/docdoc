...
Appended by `/speckit-converge` on 2026-08-24, after Phase 9 closed at 123/123 with a green gate and
SC-013 verified from a fresh clone.

The shape of this round differs from the last one. Phase 9 found code that did not exist or did the
wrong thing; seven of these nine are **documents drifting from code that works** — the amendments made
mid-implementation updated the artifacts a reader was looking at and missed the ones they were not.
That is worth naming, because it is the failure Principle X already describes for the layer chain: a
design a reader must reconstruct by choosing which of two disagreeing documents to believe is not a
governing one.

- [X] T124 Bring `data-model.md` up to the amendments the rest of the feature already took. Two places disagree with the spec they serve. **Job status** (`data-model.md` §`Job`) still states a closed set of `SUCCEEDED` and `UNAVAILABLE`, while FR-035 as amended, ADR-0010's amendment of 2026-08-24, `contracts/http-api.md` §3, and `docdoc/api/models.py::JobStatus` all carry three — `UNKNOWN` for an identity that is not well-formed, because an append-only store with no tombstones cannot tell "never produced" from "cleared" and must not pretend to. **`ErrorBody`** is given as `type`, `stage`, `message`, `detail`, while FR-066 requires a mid-run failure to carry the per-stage outcomes and the completed stages' results — which the code does, and which is the only place a partial result can appear once a failed run has no terminal artifact and therefore no job. The field also serialises as `class` rather than `type`. The CHK019 and CHK021 resolutions amended spec.md, http-api.md, ADR-0010, research.md and tasks.md; this is the artifact they missed (FR-035, FR-066, contradicts)
- [X] T125 Correct the structure block in `plan.md`, which names four paths that do not exist: `src/docdoc/pipeline/run.py` (the module is `runner.py`, deliberately — a module named `run` beside a re-exported function named `run` is shadowed by the package `__init__`), and the three test files whose assertions live elsewhere — `test_http_parity.py` and `test_http_limits.py` inside `tests/contract/test_http_contract.py`, `test_pipeline_observe.py` inside `tests/integration/test_no_leak.py`. T123 recorded this mapping for `tasks.md` and could not touch `plan.md`, which converge may not modify either (plan: structure block, partial)
- [X] T126 Restate SC-015 against something it can measure, or move it to the path it describes. As written — "the evaluation of the committed golden set, run through the command line with the store enabled, performs zero repeated parses across repeated runs" — it is satisfied trivially and measures nothing: `docdoc eval` loads a golden set and a **committed prediction set** and scores them, so it parses no document whether or not a store is configured. The repeated, billable cost the criterion exists to remove is on the **recorder** path, which is what `tests/integration/test_eval_cost.py` actually exercises and what `docs/concepts/pipeline.md` describes. A criterion that passes without exercising its subject is worse than an absent one, because it reports coverage it does not have (SC-015, partial)
- [X] T127 Assert that a run which *raises* still emits one `pipeline.stage` event per stage that ran. T116 fixed this — a `PipelineError` or `ArtifactError` propagating used to emit **zero** events because `_emit` sat only on the return path — and the fix was confirmed by hand at 0 → 3 events over a store with one corrupt artifact. Nothing in the suite pins it. The property had already regressed unnoticed for an entire milestone, and the runs most worth having events for are disproportionately the ones that failed (FR-045, missing)
- [X] T128 Make FR-030 machine-checked instead of asserted in prose. "The command line MUST NOT contain extraction, grounding, or validation logic", and `contracts/cli.md` §6 sharpens it to "a behaviour reachable only through the command line is a bug" — with nothing checking either. A forbidden-imports contract in `pyproject.toml` naming the three stage entry points (`docdoc.extraction.extract`, `docdoc.grounding.ground`, `docdoc.validation.validate`) as unreachable from `docdoc.cli` expresses it exactly: the CLI may read result *models*, which is why `cli/render.py` legitimately imports `ExtractedValue`, and must reach a stage only through `docdoc.pipeline`. Every other boundary in this project is a build failure rather than a sentence (FR-030, missing)
- [X] T129 Document `docdoc inspect --result PROCESSING_ID` outside the contract that specifies it. Added in Phase 9 to close FR-026's asymmetry with `GET /v1/jobs/{id}/result`, it appears in `contracts/cli.md` §1 and in neither `README.md`'s command block nor `docs/concepts/`. FR-056 requires every new capability to ship with documentation and at least one runnable example, and a capability documented only in a spec directory is one no reader of the project will find (FR-056, partial)
- [X] T130 Decide how FR-047's counters are reached, and say so. `docdoc.pipeline.observe.counters()` returns the executed/reused/failed/skipped totals, and `docdoc.pipeline.__all__` does not re-export `observe` — so the process-level half of "the system MUST expose counters and latency measurements per stage" sits behind a submodule the package facade does not advertise. The per-run half is on `PipelineResult.cost_summary()` and is documented. Either re-export `observe`, or record that the counters are deliberately a submodule-level concern and document the import path (FR-047, partial)
- [X] T131 Record `src/docdoc/artifacts/paths.py` and `src/docdoc/api/settings.py` in the design artifacts, or fold them away. Neither is named by `plan.md`'s structure or by `data-model.md`, and both were added in Phase 9 for a stated reason: `paths.py` applies the directory mode per level because `mkdir(parents=True, mode=…)` applies it to the leaf only, which had left both store roots world-readable against FR-044; `settings.py` holds the HTTP interface's configuration *names* outside the module that imports FastAPI, because the check asserting every documented setting exists runs on a base install with no extras. Confirm each against Constitution XI's "every abstraction MUST have a concrete, present-tense reason to exist" and add them where a reader would look for them (Constitution XI, unrequested)
- [X] T132 Reconcile `SubmittedDocument`. `data-model.md` specifies one type carrying `blob_id`, `size_bytes`, and `media_type`, serving both the submission response and the body of `GET /v1/documents/{id}`; the code has two — `SubmissionResponse` and `BlobMetadata` — which differ in that the metadata's `media_type` is optional, because a blob read back from the store may not be re-detectable. That is a defensible reason to split them and it is recorded nowhere. Either collapse them or state the divergence in `data-model.md` (data-model: `SubmittedDocument`, partial)
      → **Divergence recorded rather than collapsed.** The two types differ in exactly one field and the difference is real: a submission has just seen the bytes so its `media_type` is known, while a metadata read of a blob already in the store may not be able to re-detect one. Collapsing them would mean either an optional field on the response that is never absent, or a required field on the lookup that cannot always be filled. `data-model.md` now says so.

---

## Phase 11: Convergence

Appended by `/speckit-converge` on 2026-08-24, after Phase 10 closed at 132/132 with a green gate.

This round went looking in the one place the previous three had not: the **edge cases**, one at a
time, and the per-stage folded-input table of ADR-0003. Both findings above HIGH come from there, and
they are related — the second is most of why the first survived three passes.

- [X] T133 Let a caller give the pipeline grounding and validation options, or record in the spec that it cannot. `run()` accepts `options` for the **parse** stage only; `_ground()` and `_validate()` call their layers with defaults, so `GroundingOptions` and `ValidationOptions` can never be supplied. ADR-0003's table (`:88`) makes the enabled rule **ids** and the grounding policy the only inputs that fold into those two stages' options hashes and are not already inside `schema_hash` — which means they are the only changes that invalidate ground or validate **alone**. Neither is reachable. So FR-013's "reuse is partial and per-stage" is demonstrable in one direction (edit the schema or the model, the parse is reused) and unreachable in the other (invalidate validate without invalidating extract), and US2's third acceptance scenario — "when only a validation rule changes, the parse is reused, and the run recomputes exactly the stages whose recorded inputs moved" — cannot be written as a test because there is no lever to move. Adding two keyword arguments is small; deciding they are out of MVP scope is also defensible, and either way `contracts/pipeline-api.md` §1 should say which (FR-013, US2/AC3, missing)
      → **All three added** — `extraction_options`, `grounding_options`, `validation_options` — threaded to both the stage call and its identity computation. FR-013's per-stage claim is now demonstrable for all four stages: a stricter grounding policy recomputes validate alone *and moves the verdict*, a different threshold recomputes ground and validate, a different temperature recomputes everything below the parse.
      → **One correction to this task's own framing.** It said US2/AC3 "cannot be written as a test". That was overstated: rules live in the schema, so ADR-0003 makes a rule edit invalidate extraction onward while reusing the parse — which the schema-change test already covers. What was genuinely unreachable is narrower and wider at once: `enabled_rules` and the grounding policy invalidate their stage *alone*, and `ExtractionOptions` was unreachable too, so **three** of four stages had no lever rather than two.
- [X] T134 Inject a failure at the **ground** and **validate** stages and assert the preceding results survive. Every failure test in this repository fails at extract, because an unresolvable schema is the convenient lever: `tests/integration/test_pipeline_failures.py` has four cases and all four stop at `Stage.EXTRACT`, and the one test that reaches `Stage.GROUND` (`test_pipeline_errors.py::test_a_grounding_error_raised_during_validation_is_a_grounding_error`) asserts the *attribution* and never looks at whether the extraction and grounding results came back. SC-012 says "a run that fails at stage *n* returns the results of all *n−1* preceding stages in **100% of cases**", and *n*=3 and *n*=4 have never been exercised — which is also most of the reason T133 went unnoticed through three convergence passes (SC-012, missing)
- [X] T135 Finish T109, which was marked complete on a third of its claim. It asked for "a failure injected at each of stages 2, 3, and 4" returning the preceding stages' results, **and** for the same failure over the CLI and over HTTP to agree on the stage, the error class, and which results survive. What exists is `test_a_mid_run_failure_carries_the_completed_stages`, covering stage 2 over HTTP alone. The spec's Edge Cases are stricter than the task: "the command line, the HTTP interface, **and the recorder** must agree on what is returned: the stages that succeeded, the stage that failed, and its typed error class — never the exception message, which can quote the document." Three surfaces, and no test compares any two of them. The recorder's rendering has its own tests (`test_record_and_score.py`) and has never been checked against the other two (T109, spec §Edge Cases, partial)
- [X] T136 Wire or drop the `options` parameters on `ground_artifact_id` and `validate_artifact_id` in `src/docdoc/pipeline/plan.py`. Both accept a per-stage options object and both are called by `runner.py` (`:348`, `:367`) without one, because no caller can supply one — see T133. A parameter that cannot be exercised is worse than an absent one: it reads as a supported degree of freedom and is dead. If T133 adds the plumbing these become live; if T133 records the limitation instead, these should go, and the identity functions should say plainly that the MVP's grounding and validation options are fixed (Constitution XI, unrequested)
      → **Wired, not dropped**, as T133 resolved. `extract_artifact_id` already took one; `ground_artifact_id` and `validate_artifact_id` now receive what the runner passes to the stage, which is the only arrangement that works — an option given to a stage and not to its identity would store a result under an identity that does not describe it, and every later run would miss forever. `test_the_same_options_twice_reuse_everything` is the assertion that the two agree.

---

## Phase 12: Convergence

Appended by `/speckit-converge` on 2026-08-25, after Phase 11 closed at 136/136. The gate was re-run
in full for this pass and is green: the suite and the perf suite both exit `0`, `mypy` is clean on 13
kernel files, `ruff` is clean, `lint-imports` reports 8 contracts kept and 0 broken, the working tree
is clean, and every file of all four new packages is tracked.

Both findings are in one place, and it is the place four passes did not think to compare: the
**configuration vocabulary**. Every previous round compared a document against code, or code against
a requirement. Neither finds this one, because the settings are each individually correct, each
individually documented, and enforced in both directions by
`tests/unit/test_documented_api_references_resolve.py` — which checks that a setting the code reads
is named in a document and that a setting a document names is read by the code, and never that the
*flag* beside it exists. FR-031 is a claim about the relationship between two vocabularies, and
nothing had been comparing the two as sets.

- [X] T137 Give `DOCDOC_MAX_DOCUMENT_BYTES`, `DOCDOC_MAX_PAGES`, and `DOCDOC_MATCH_VIEW_CACHE` command-line flags, or narrow FR-031 to say which settings are environment-only and why. FR-031 requires that "where configuration is already environment-driven, the command line MUST accept the same settings as explicit arguments", and `src/docdoc/cli/config.py` defines six flags — `--json`, `--schema-path`, `--adapter`, `--echo-fixtures`, `--store`/`--no-store`, `--verify-cache` — none of which is one of these three. All three govern a command-line run: the two size limits are read by `ingest.Limits`' field defaults (`src/docdoc/ingest/source.py:62`) on the path every `docdoc parse` and `docdoc extract` takes, and the cache bound is read by `grounding/view.py:177` on the path every grounding stage takes. The CLI never constructs a `Limits` at all, so there is no seam to hang a flag on yet. Two of the three were **created by this milestone**: T121 added them in Phase 9 to close FR-039's "configurable maximum document size", which until then was configurable only by a caller importing the library — the fix closed FR-039 and opened this. Narrowing is defensible for the cache bound, which is a tuning knob rather than a policy; it is harder to argue for a size limit, since FR-039's whole point was that an operator running the command should be able to set one. `checklists/interfaces.md` CHK018 asks whether FR-031's rule is one-directional and assumes this direction is the required one; whichever way this goes, that item can be closed with it (FR-031, partial)
      → **Both, split by whether the setting can change an invocation's outcome.** `--max-document-bytes` and `--max-pages` added and threaded into `parse` and `extract` — the flags carry FR-039's promise to anyone not importing the library, which is who FR-039 was for. `DOCDOC_MATCH_VIEW_CACHE` is recorded as environment-only instead: a command-line run grounds one document once, so the cache holds a single entry and every bound above zero behaves identically, and the bound is read when `docdoc.grounding.view` is imported — so a flag parsed afterwards would need new machinery to move a number that changes nothing.
      → **Two more exclusions were found while listing the first.** `DOCDOC_MAX_REQUEST_BYTES` caps an HTTP request body and the command line reads none. `DOCDOC_GEMINI_MODEL` and the two Azure credentials are per-provider: a `--gemini-model` flag would put one vendor's vocabulary on a provider-agnostic command line, and a credential must never be an argument at all, because `argv` is readable by every process on the host — a worse exposure than the variable it would replace, and against FR-042 in the one direction nobody checks.
      → **FR-031 amended** to require a flag for the settings that can move an invocation's outcome, and to require every exclusion to carry its reason. Recording the reason is the part that matters: an exclusion nobody wrote down is indistinguishable from an oversight, which is exactly how two of these came to be documented as having flags they did not have.
      → **`tests/unit/test_cli_config_vocabulary.py` is what keeps it true.** It compares the settings and the flags **as sets**, which is the comparison nothing was making. `test_documented_api_references_resolve.py` already checked settings against *documents* in both directions and would have caught a setting nothing documents or a document naming nothing — so each half was individually verified, and the relationship between them was not. That is why four convergence passes read past this.
      → A non-positive limit is an invocation error (`64`) naming the flag that was typed, rather than a pydantic `ValidationError` naming `max_size_bytes`, a field nobody typed.
- [X] T138 Reconcile `README.md` and `contracts/cli.md` §4 with whatever T137 decides — both currently assert the property T137 says is not held, and the contract's table is wrong in the other direction too. `README.md:239` reads "Every setting gains a flag of the same meaning on the command line — `--schema-path`, `--adapter`, `--store` — so there is no second vocabulary to learn", and `contracts/cli.md` §4 opens "Every environment setting that already exists keeps its name and gains a flag of the same meaning (FR-031). No second vocabulary." Neither sentence is true of the three settings in T137. Independently of T137, the contract's table lists three rows and omits `DOCDOC_ECHO_FIXTURES` → `--echo-fixtures`, a flag that **does** exist and is the one the offline path of FR-029 turns on — so a reader of the CLI contract cannot discover the setting the contract's own §5 depends on. `README.md:224` also says "Three more exist and are rarely worth touching" above a list of five, having been written when there were three and not recounted when T121 added two. A count nobody maintains is the same defect as a status field nobody maintains, which this spec's own Status block already argues at length (FR-031, `contracts/cli.md` §4, contradicts)
      → **All three corrected, and the contract now carries the exclusions as a second table** so a reader learns which settings have no flag from the document that specifies the flags, rather than by trying one. `--echo-fixtures` and `--json` are listed, the two new limit flags are listed, and `README.md`'s count reads five because five is what is in the list beneath it.
      → Neither document any longer claims a property no test held. The claim each now makes — that the flags and the settings are two views of one vocabulary — is the claim `tests/unit/test_cli_config_vocabulary.py` asserts, so the next time one of them drifts the suite says so instead of a convergence pass five rounds later.

---

## Phase 13: Convergence

Appended by `/speckit-converge` on 2026-08-25, after Phase 12 closed at 138/138 with a green gate:
the suite, the perf suite, `ruff`, `mypy`, `lint-imports` 8/0, coverage 93.79% against the 93% floor,
and the offline suite passing on a base install built in a clean venv.

Nothing above LOW except the first, and the first exists **because of Phase 12**. T137 gave the
document-size limits a command-line lever, which is what FR-039 wanted; a lever a user can move is
also a lever that can move something it must not. That is not an argument against the change — it is
the ordinary cost of making a setting reachable, and the remedy is one test.

Two edge cases probed directly this round came back clean and are recorded so the next pass does not
re-probe them: the missing-credential failure names `GEMINI_API_KEY`/`GOOGLE_API_KEY` rather than the
document, arriving as a typed `ModelProviderError` at exit `2`; and both new limit flags genuinely
refuse, `--max-pages 2` against the three-page fixture exiting `2` with `UnsupportedDocumentError`.

- [X] T139 Pin that the limit flags move no identity, and that `--max-pages` refuses. `--max-document-bytes` and `--max-pages` became user-reachable in T137, and nothing in the suite asserts that moving either leaves `document_id` and the terminal identity where they were. `tests/unit/test_stage_identity.py` enumerates what must stay out of an identity — durations, `request_id`, retry counts, transport settings — and never mentions limits, because until Phase 12 no caller outside the library could set one. `tests/unit/test_limits.py` covers enforcement and never mentions an identity. The property does hold: three `document_id`s taken across `--max-pages 500`, `--max-pages 999`, and no flag are byte-identical, checked by hand during this pass. By hand is not the suite, and this is the failure mode that would not announce itself — a limit folded into the parse options hash would give every deployment with a different cap a disjoint cache, and each one would look correct from the inside. Add the assertion beside FR-060's existing ones rather than in a new file. `tests/unit/test_cli_config_vocabulary.py` also pins `--max-document-bytes` enforcement and not `--max-pages`; the three-page fixture `tests/fixtures/pdf/mixed_pages.pdf` is what closes that half (FR-060, SC-010, missing)
- [X] T140 Stop `docdoc store --help` promising an action that does not exist. Both the top-level help and the subcommand's own describe it as "**inspect** and clear the artifact store", and there is no inspect action: `src/docdoc/cli/__init__.py:213` raises for any action but `clear`, and `contracts/cli.md` §1 names only `docdoc store clear [--stage STAGE]`. Help output is not an internal comment — it is the surface a user reads before reading anything else, and it is the one piece of documentation that ships inside the program. Dropping "inspect and" is the small fix; adding a `docdoc store stat` is the other, and it would need a contract line first, because FR-019 deliberately grants two subsets and no query language. This is the same defect class as T138 and was found the same way: by running the command rather than by reading about it (FR-019, `contracts/cli.md` §1, contradicts)
- [X] T141 Validate `--stage` before checking whether a store is configured. `src/docdoc/cli/commands/store.py:62` validates the name against the four stages and its docstring makes the case plainly — `clear(stage="extarct")` "removes nothing and reports success — a typo that reads as a completed teardown is exactly the kind of silence that makes a later cache incident inexplicable". The `has_store` early return at line 40 sits in front of it, so `docdoc store clear --stage extrat --no-store` exits `0` reporting only that no store is configured, and the typo is never mentioned. With a store configured the same typo correctly exits `64`, so the check works and is merely unreachable on one path — which is the worse shape, because the behaviour a user sees depends on a setting unrelated to the mistake they made. An argument is well-formed or not on its own terms; judging it first costs one moved block (FR-019, `cli/commands/store.py:62`, partial)

---

## Phase 14: Convergence

Appended by `/speckit-converge` on 2026-08-25, after Phase 13 closed at 141/141 with a green gate
and coverage at 94.41%.

This round ran the product instead of reading about it, and that is how the first finding surfaced.
Five passes checked whether the code did what the tests said; this one checked whether the **spec's
headline promise** matched what a user actually sees on the committed fixture. It does not, and the
gap has been sitting under a passing test since Milestone 7 began — because the test that carries
SC-001's name asserts a narrower claim and argues the narrowing in its own docstring, where the spec
cannot see it.

- [X] T142 Restate SC-001 against what the product can honestly promise, and reconcile the two places that repeat it. As written it reads "100% of extracted values carry a page and, where geometry exists, a bounding box" — and the qualification attaches to the *bounding box*, so the literal claim is that **every** extracted value carries a page. `docdoc inspect tests/fixtures/pdf/digital_invoice.pdf --schema invoice@1` on the offline path returns 13 values of which **5** carry one. The other eight are `ungrounded`, and the output says so plainly (`page` renders `-`, `grounding` renders `ungrounded`) — which is not a defect but the requirement: Principle II forbids emitting a location for a value the grounder could not find, and fabricating a page to satisfy this criterion is precisely the failure that principle exists to prevent. So SC-001 as written is unsatisfiable without violating the constitution, on any document where grounding is imperfect. `tests/integration/test_cli_offline.py:89` already resolves the tension the only way it can — it filters to `grounding in {"exact", "fuzzy"}` and its docstring argues that "SC-001's 'where geometry exists' is exactly that qualification". That argument may well be the intended reading, but it lives in a test file, and a measurable criterion that means something other than what it says is the defect T126 fixed for SC-015: a criterion whose passing does not mean what a reader thinks it means is worse than an absent one. Amend SC-001 to quantify over *grounded* values (or to state the grounding rate it expects and why), and fix the two artifacts that repeat the stronger claim: **US1/AC1**, which requires "one entry per schema field with its value, its grounding status, its page, and its bounding box", and **quickstart §1**, whose Expected line reads "one line per schema field carrying its value, its verdict, its page, and its bounding box" — a reader running that scenario sees eight rows without either and reasonably concludes it failed (SC-001, US1/AC1, quickstart §1, contradicts)
- [X] T143 Record `ArtifactRef` in `data-model.md`, or say there that it is subsumed. The spec's Key Entities names it — "an artifact's identity together with the stage that produced it and the identity of its input — the edge of the chain, which is what makes a derivation explainable" — and it has no referent: no class defines it and `data-model.md` never mentions it. This is not a functional gap; the edge exists, on `ArtifactEnvelope`, whose documented table carries `stage` ("Which stage produced it") and `input_artifact_id` ("The previous link in the chain") with exactly those semantics, and `test_explain.py::test_the_chain_reaches_the_source_blob_in_four_hops` walks it. What is missing is the one line saying so. Every comparable divergence in this feature *is* recorded — `RunLimits` as "an alias for the existing `ingest.Limits`", the "What is deliberately not modelled" list covering the second `Stage` enum and the second logging payload shape, and T132's `SubmittedDocument` split — which is what makes this one an omission rather than a convention (spec §Key Entities, partial)

---

## Phase 15: Convergence

Appended by `/speckit-converge` on 2026-08-25, after Phase 14 closed at 143/143 with a green gate.

One finding, and it is the previous task's own residue. T142 amended SC-001 and reconciled the three
artifacts that restated it — US1/AC1, US1's Independent Test, quickstart §1 — and four more say it
still. That is the failure Phase 10's preamble already named: *an amendment updates the artifacts a
reader was looking at and misses the ones they were not.* It has now happened three times in this
milestone (the CHK019/CHK021 pass missed `data-model.md`, T137 missed `README.md` and
`contracts/cli.md`, and T142 missed these), which is worth stating plainly in case a later reader
wonders whether it was carelessness or a pattern. It is a pattern, and its cause is that no check
exists which reads a claim in one document and looks for the same claim in the others.

Everything else this round came back clean, and is recorded so a later pass need not redo it: all
sixteen success criteria trace to a live verifier, all five HTTP endpoints appear in the generated
OpenAPI schema, and SC-001 as amended now measures 100% on both halves — 5 of 5 grounded values
carrying a page and a box, 8 of 8 ungrounded carrying neither, and all 8 still carrying their value.

- [X] T144 Reconcile the four remaining statements of the claim SC-001 no longer makes. Two are quantified and carry the exact defect T142 removed: **`spec.md:51`**, in this milestone's own framing paragraph — "What it must produce is the Definition of Done stated at the founding of the project: a PDF goes in one end of a command, and a human can ask **any** extracted value which page and which rectangle it came from" — and **`CHANGELOG.md:14`**, which repeats that sentence to the public. On the committed fixture a human can ask 5 of 13, and asking the other 8 correctly returns "ungrounded, no location", because Principle II forbids inventing one. Two more are command-listing comments that read as the same promise in shorter form: **`README.md:159`** and **`contracts/cli.md:13`**, both `# every value, its verdict, its page, its rectangle`. These are weaker — they can be read as naming the table's columns rather than guaranteeing every cell is filled — but they are the first thing a reader of either document sees about `inspect`, and the charitable reading is one this milestone has already declined to rely on once. Amend all four to the shape SC-001 now carries: a location for every value the grounder located, and an explicit ungrounded marker for every one it did not. The Definition of Done is not weakened by this and should not be written as though it were — *provenance for every located value, and honesty about the rest* is the promise the project actually keeps, and it is the stronger claim, because the alternative is a system that always answers and is sometimes wrong (SC-001, spec §51, contradicts)
