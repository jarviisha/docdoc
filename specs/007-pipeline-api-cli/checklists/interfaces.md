...
- [x] CHK018 Is the direction of FR-031's rule stated — every environment setting gains a flag, but is the converse required? `--verify-cache` has no environment setting, which is permitted only if the rule is one-directional. [Clarity, Spec §FR-031, Contract cli.md §4]
      → **One-directional, and now said so — after the other direction turned out to be false in three places.** FR-031 amended 2026-08-25 (T137): a setting that can change what an invocation does gains a flag; a flag need not gain a setting. `--verify-cache` and `--json` are per-invocation choices, not deployment configuration, and `contracts/cli.md` §4 now lists them as flags with no setting rather than leaving them as unexplained rows.
      → The item was raised as *clarity* and was hiding a *correctness* gap. While it sat open, `DOCDOC_MAX_DOCUMENT_BYTES`, `DOCDOC_MAX_PAGES`, and `DOCDOC_MATCH_VIEW_CACHE` had no flag while both `README.md` and this contract asserted that every setting had one. The first two now have `--max-document-bytes` and `--max-pages`; the third is recorded as environment-only with its reason, along with the request cap and the per-provider settings.
      → What keeps it closed is `tests/unit/test_cli_config_vocabulary.py`, which compares the settings and the flags **as sets**. Nothing had been doing that: the existing check compared settings against *documents* in both directions, which is why four convergence passes read past it.
...
- [x] CHK024 Is a result retrievable **by identity** from the command line, as it is over HTTP? FR-026 requires a command to "inspect a result's values and their locations", while `inspect` is specified to take a file and a schema and re-run the pipeline. Someone holding a `processing_id` from a log has an HTTP path and no CLI path. [Consistency, Spec §FR-026, Contract cli.md §1, Contract http-api §1]
      → **Closed by `docdoc inspect --result PROCESSING_ID`** (2026-08-24, converge task T120). Three store lookups walking the chain the way the HTTP interface does, since each artifact records its input's identity — which is the reachability property FR-022 already required for a future collector, paying for itself here. Absent results are reported and never recomputed (FR-036). `cli.md` §1 documents the two forms and the rule that exactly one may be given.
