...
**14 targeted mutations, 11 caught, 3 survived — a mutation score of 79%.** No finding here is a
code defect: the implementation behaved correctly under every probe. What the survivors expose is
three claims that nothing holds.

Caught, and worth recording as what the suite *does* protect: verdict precedence, a warning
rejecting a document, per-severity counts, geometry and pages copied rather than recomputed,
requiredness reading presence rather than content, length and entry-count bounds, `enum` compared
through the field's declared type, **an absent sum operand counted as zero**, and grounding checks
emitted for absent values.

**T108 is the one to do first**, and not because of its severity alone. A test that reads as
coverage and provides none is worse than an absent test: it stops the next reader from looking.

- [X] T108 Fix `test_a_path_the_walk_never_produced_sorts_last_rather_than_first` in `tests/unit/test_finding_order_is_total.py`, per FR-043 (partial). It passes under the exact mutation it exists to catch — making an unknown path sort *first* changes nothing, because the stray record is compared against `number#required`, whose walk position is already `0`. Both tie on position, both have no entry index, and the comparison falls through to `check_id`, where `"rule:ghost@nowhere"` happens to sort after `"number#required"`. The assertion has been succeeding for a reason unrelated to what it claims. Compare the stray against a record from the **middle** of the walk (`total#required`, say) so that position is what decides, and assert the ordering in both directions. Verify the fix the way the defect was found: set `position = 0` in `sort_key` and confirm the test now fails
- [X] T109 [P] Pin whole-value matching on both sides in `tests/unit/test_constraints.py`, per FR-024 (partial). `test_the_whole_value_must_match` uses `"xxINV-2026yy"` — junk before *and* after — so it only ever exercises the trailing half, and a matcher that tolerated a leading prefix would pass it. Split it: `"INV-2026yy"` (trailing only), `"xxINV-2026"` (leading only), and the both-sides case. The leading-only case is the one that currently survives mutation, and it is the likelier bug in practice — a matcher written with `search` rather than `fullmatch` anchors at the end more often than at the start
- [X] T110 [P] Decide what the entry-index term in `sort_key` is for, in `src/docdoc/validation/verdict.py` and `tests/unit/test_finding_order_is_total.py`, per FR-043 (partial). Dropping it breaks nothing, and the docstring explains why — "the walk position already accounts for entry order, so the middle term is belt and braces" — which means no path the walk produces can reach it. Either exercise it with a synthesized anchor that the walk did not produce but that carries an index (the case a future rule kind anchored at a computed path would create), or remove it and record that the walk position is the whole ordering. Do not leave code whose stated purpose no input can reach: the next reader cannot tell whether it is dead or load-bearing

---

## Phase 14: Convergence (seventh pass)

Mutation applied to the two modules the sixth pass did not reach: `enumerate.py` and
`identity.py`. **13 mutations, 10 caught, 3 survived.** No code defect again — but the first
survivor is the same species as the self-deceiving test Phase 13 fixed, sitting in the
assertion that matters most to ADR-0003.

Caught, and worth recording as held: `group_absent` inverted, a group never reported absent,
walk order replaced by alphabetical order, entry counts forced to zero, a missing declared
field silently skipped, four separate identity terms omitted from `options_hash`, and the
validator version dropped from the artifact id.

- [X] T111 Assert the artifact **chain**, not that one function calls another, per FR-047 and ADR-0003 (partial). Truncating `grounding_artifact_id` to twenty characters inside `validation_artifact_id_for` survives the whole suite, because `test_it_chains_from_the_grounding_artifact` in `tests/contract/test_validation_identity.py` recomputes the expected value **with the same function** — so a mutation inside it moves both sides of the assertion equally. The test proves `validate()` uses that helper and nothing about what the helper depends on. Replace it with two assertions that cannot be circular: call `validation_artifact_id_for` twice with two grounding artifact ids that differ **only in their last characters** and require the results to differ; and confirm the same holds end to end, by validating one extraction against two grounding results whose ids differ. Verify the fix the way the defect was found — truncate the id in `identity.py`, watch the test fail, restore
- [X] T112 [P] Cover the third direction of the shape refusal in `tests/unit/test_check_enumeration.py`, per FR-018 (partial). `TestShapeRefusals` covers a scalar where a **group** is declared and a group where a **repeating group** is declared. A dict or a tuple where a **scalar** is declared is never tried, and disabling that branch of `_field` survives the suite. Add both: a group payload under a scalar field, and a repeating-group tuple under a scalar field, each refused with the field named
- [X] T113 [P] Rule on the defensive `sorted(enabled_rules)` in `src/docdoc/validation/identity.py`, per FR-048 (partial) — the same question T110 answered for the entry-index term, and it deserves the same treatment rather than a second judgement call. Dropping the sort breaks nothing, because `enabled_names()` hands it an already-sorted tuple. Either assert directly in `tests/contract/test_validation_identity.py` that two calls differing only in the *order* of `enabled_rules` produce one hash — which is what makes the ordering of a caller's set irrelevant to identity — or remove the sort and state that callers must pass it sorted. Prefer the assertion: `options_hash_for_validation` is reachable from outside `validate()`, and a hash that depended on a set's iteration order would be a reproducibility bug of exactly the kind FR-051 forbids
