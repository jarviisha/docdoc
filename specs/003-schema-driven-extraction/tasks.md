...
- [ ] T053 [US3] Implement `src/docdoc/extraction/adapters/anthropic_messages.py` — request construction from the `ResponseShape`, provider-enforced structured output, and response mapping. The SDK import lives here and nowhere else (R1, R2, FR-023)
- [ ] T054 [US3] Implement stop-reason branching in `src/docdoc/extraction/adapters/anthropic_messages.py` — check the stop reason before touching content; map a refusal to a permanent `ProviderError` carrying the provider's stated category, and a truncated response to the output-budget `ExtractionError` of FR-030 rather than to a retry (R12, R14)
- [ ] T055 [US3] Implement error translation in `src/docdoc/extraction/adapters/anthropic_messages.py` — every provider exception becomes a docdoc error with `__cause__` preserved; no provider exception crosses the boundary (FR-042)
...
- [ ] T057 [US3] Implement `available()` in `src/docdoc/extraction/adapters/anthropic_messages.py` — a missing SDK or a missing credential reports unavailable **with the reason**, and `extract()` raises before any byte is transmitted (FR-028, FR-041)
- [ ] T058 [US3] Assert the absence of forbidden parameters in `tests/unit/test_anthropic_mapping.py`: the request must carry no `temperature`, `top_p`, `top_k`, or seed. On the chosen provider's current models any of them is a hard rejection, so this is a correctness test, not a style one (R4)
...
- [ ] T060 [US3] Verify the prompt cache actually reads, in `tests/integration/test_anthropic_live.py` — two documents against one schema, and the second extraction's usage must report a non-zero cache read. A zero means something volatile precedes the breakpoint, which T027 also guards from the offline side (R15)
