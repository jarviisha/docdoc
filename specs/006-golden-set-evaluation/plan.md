...
Evaluated against **constitution v1.4.0**. **Initial: PASS. Post-design re-check: PASS.**
...
| 5 | **Provider isolation (IV)** | **PASS, with the caveat that makes it real** — `docdoc.evaluation` touches no provider and the base install gains nothing. `docdoc.recording` reaches a provider only through the existing `ModelAdapter` and registry, naming no SDK type. The caveat, **corrected while implementing**: an earlier draft claimed a forbidden-imports contract could not prove this because the provider is imported inside a function. import-linter follows function-scoped imports, and the contract broke on the first module that imported `docdoc.extraction` as a package. What keeps the lower layers green is that they import extraction's *submodules*, never the package, so `extraction/__init__.py` and its adapter registry never enter their graph. `docdoc.evaluation` follows the same rule, the contract is load-bearing, and the runtime test remains for what a graph cannot see (research.md R1) |
...
| 11 | **Layer direction (X)** | **PASS, and no longer by reconciliation** — `docdoc.evaluation` above `docdoc.validation`, and `docdoc.recording` above `docdoc.evaluation`. Milestones 4 and 5 both had to argue that Principle X's chain named neither of the layers they added and that ADR-0003's finer-grained stage chain was consistent with it. Constitution v1.4.0 ends that: Principle X now states the chain that exists, names Evaluation in it, lists Recording as planned, and makes the `pyproject.toml` contract authoritative. This gate is now read off the governing document rather than reconstructed from three `research.md` files. Evaluation is **not** a stage in that chain — it consumes the validation artifact and produces nothing a document-processing stage consumes — which is why its identity is named `report_id` and not `artifact_id` (EVA-24) |
...
7. **Import extraction's submodules, never the package — and this was learned the hard way.** The plan
   originally asserted that forbidden-imports contracts cannot see a provider reached through
   `docdoc.extraction`, because the SDK is imported inside a function. import-linter follows
   function-scoped imports, and the first version of `evaluation/predictions.py` broke the contract on
   exactly that chain. The rule that keeps grounding and validation green is submodule imports:
   `docdoc.extraction.value`, not `docdoc.extraction`. Evaluation follows it, the contract is real,
   and the runtime test covers what no graph can (research.md R1).
...
The gate stays advisory until the dataset reaches that size, which is exactly what constitution v1.4.0
