...
- [X] T004 [P] Build the typesetting fixtures in `tests/fixtures/grounding/`: documents carrying an *fi* ligature, a soft hyphen mid-word, a lowercase word broken by a line-break hyphen, an identifier (`INV-2024-001`) broken by a line-break hyphen, a compound word (`well-known`) broken by a line-break hyphen, non-breaking / narrow / figure spaces, and a combining-mark sequence. No builder script: `make_fixtures.py` regenerates *binary* fixtures so a reviewer can see inside a blob, and these are string constants with nothing to regenerate and nothing hidden (reconciled by T062)
...
Appended by `/speckit-converge` after implementation. Nothing is `missing` — every functional
requirement has working code behind it. What remains is undefended behaviour, two artifacts nothing
reads, and one plan claim that outran the implementation.

- [X] T059 Add the score-incomparability note to `Alternative.score`'s field description in `src/docdoc/grounding/result.py` per FR-031 (partial). T043 put it on `GroundingOutcome.score` with the argument that FR-031 requires it *wherever the score is exposed* and the generated schema is one of those places. `Alternative.score` is exposed in that same schema and carries nothing — verified with `model_json_schema()`. Extend `tests/unit/test_no_view_offsets_escape.py` or `test_fuzzy_tier.py` to assert both descriptions exist, so the pair cannot drift again
- [X] T060 Thread document and field identity into the offset-map guards in `src/docdoc/grounding/offsets.py` per FR-043 (partial). `OffsetMap._validate` and `_map` raise `GroundingError` with `document_id=None` and `field_path=None`, so the one failure mode that means "the highest-risk component broke" names neither the document nor the value. These are defensive checks that should never fire, which is exactly why they must be legible when they do
- [X] T061 Decide what `MatchView.view_id` is for, in `src/docdoc/grounding/view.py` per FR-020 (unrequested). It is computed on every run — a `sha256` per grounding — and read by nothing outside its own test. FR-020's stated purpose, "so that two runs can establish they compared against the same thing", is already met without it: provenance carries `document_id` and `match_view_version`, and `view_id` is derivable from exactly those two. Either record it in `GroundingProvenance` so a consumer can use it, or remove it and document the derivation. Do not leave a field that looks meaningful and reaches no one
- [X] T062 Reconcile the `tests/fixtures/make_fixtures.py` touch-point (contradicts). plan.md's "Existing files this feature edits" list says it was "extended for the grounding fixtures" and T004 asked for "a builder script so the fixtures are regenerable"; the file is untouched. The grounding fixtures are Python string constants, which need no builder — regenerating a literal is meaningless. The likely correct fix is to correct plan.md rather than write a builder, but the artifact and the tree currently disagree and one of them is wrong
- [X] T063 [P] Add tests for the spec's enumerated edge cases in `tests/unit/test_grounding_edge_cases.py` (partial). All nine were **verified during convergence to behave correctly**, so this defends working behaviour rather than fixing broken behaviour: an empty document; a whitespace-only document; a claim longer than the whole text; a claim equal to the whole text; a claim resolving across a page boundary (returns two pages); characters outside the Basic Multilingual Plane; a right-to-left script; a claim occurring hundreds of times (alternatives stay capped at five); a header repeated on every page; and the threshold configured to 0.0 and to 1.0, both of which are legal and both of which change every outcome
- [X] T064 [P] Exercise the third geometry state in `tests/unit/test_geometry_unavailable.py` per GRD-17 (partial). The file's own docstring names three states and asserts two. `geometry == ()` — geometry exists and the range covers no tokens — is reachable: `Document.locate()` returns `()` for a whitespace-only range, confirmed during convergence. Untested, the distinction between "unavailable" and "nothing there" rests on one of its two halves
- [X] T065 [P] Remove `find_fuzzy` from `__all__` in `src/docdoc/grounding/match.py`, or state why it is public (unrequested). It is imported nowhere outside its own module. `resolve`, `select`, `MAX_ALTERNATIVES`, and `Candidate` all have real consumers; this one advertises a surface nothing uses

**Dependencies**: T059–T062 are independent of each other and of the rest. T063, T064, and T065 are
mutually parallel and depend on nothing.

**Not appended, and why.** SC-003 asks for "hand-verified expected locations" and
`test_ground_real_pdf.py` asserts `document.text[span] == claim` rather than a committed table of
offsets. That was considered and is **not** a gap: a wrong offset returns different text, so the
assertion does discriminate, and the multi-occurrence case — the one a read-back cannot settle — is
pinned separately in `test_exact_tier.py` with explicit `TEXT.index(...)` expectations.
