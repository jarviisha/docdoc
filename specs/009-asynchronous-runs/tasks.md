...
- [X] T120 Update the `RunQueue` protocol block in `specs/009-asynchronous-runs/contracts/runs-layer.md` to the surface the code actually has (contradicts). Five of the nine documented signatures no longer match `src/docdoc/runs/queue.py`: `finish` is shown as `(run_id, outcome, *, now) -> None` and is now `(run_id, outcome, *, now, worker_id=None, only_from=None) -> bool`; `heartbeat` and `release` each gained `worker_id`; `claim` gained `max_attempts` and `submit` gained `expires_at`, both of which predate this session. **The three `worker_id` parameters are not cosmetic** — they are the entire fix for the ownership defects a review found, so the contract currently publishes the exact surface that let a superseded worker overwrite the live attempt's verdict, requeue a run another worker was executing, and extend a lease it no longer held. The file states its own policy in the sentence after the block — "The last two arrived with the code and are recorded here rather than left implicit" — so this is the document contradicting a commitment it makes about itself. Record what each new parameter is for, not only that it exists: `worker_id` is optional because callers that are not workers (`cancel`) have no claim to prove, and `only_from` exists so cancelling a queued run cannot stop one claimed a microsecond earlier
- [X] T121 Correct `attempts` in `specs/009-asynchronous-runs/data-model.md` (contradicts). Two statements are now false. The field table says "Incremented in the claim statement itself (R8)", which is half of what happens — `release` decrements it, because a graceful shutdown is not the evidence `max_attempts` bounds and counting it would make a rolling restart spend the redelivery budget of every run in flight. And the Invariants section says "`attempts >= 1` in every state except `queued`-before-first-claim", which a claimed-then-released run now violates in substance: it sits in `queued` with `attempts = 0`, indistinguishable from one that was never claimed. State the real invariant — `attempts` counts claims **currently accounted for**, not claims that ever happened — and say plainly that the two are no longer the same thing, because a reader who assumes `attempts` is a history will misread every released run. The transition table's `released` row should say the attempt is returned; it currently does not
- [X] T122 [P] Add the `cancelled` reason to the transition table in `specs/009-asynchronous-runs/contracts/runs-layer.md` (contradicts). The table is a closed enumeration — "The transitions, and the `reason` each carries" — and its terminal row reads "the error class, or `completed`". That is exactly what the code did and exactly what was wrong with it: a cancelled run carries no `error_class`, so every deliberate stop was logged as a completion and an operator counting completions counted cancellations among them. `observe.REASONS` now carries `cancelled` and `reason_for` emits it. The table should distinguish the two rows it currently merges — `cancel_requested` for the running run that was asked, `cancelled` for the one that actually stopped — since telling those apart is the whole point of having both
- [X] T123 [P] Document the document store's `503` in `specs/009-asynchronous-runs/contracts/runs-http-api.md` (partial). The `POST /v1/runs` status table gives `503` for "run-state database unreachable" and `404` for "blob unknown **or** owned by another tenant". An unreachable *document* store now answers `503` as well, mapped from `ArtifactError(reason="unavailable")` in `api/errors.status_for`. Before that it fell into the `404` row, which told a caller their document was gone when the store was merely down — the same absent-versus-unreachable conflation this milestone removed from the blob stores, surfacing one layer up. Note in passing that `not_configured` deliberately stays `500`: a store nobody configured is a deployment fault and retrying will not conjure one
- [X] T124 Justify or document `ResultNotStored` (unrequested). `runs/worker.py` introduces a terminal `error_class` that no FR calls for and that `data-model.md` does not list among the failure shapes it enumerates. It exists because both artifact stores drop a failed write and continue (FR-063) — right for the three intermediate stages, which are a cache, and wrong for the last one, whose id becomes `processing_id` and is the only handle the caller is given: a dropped write there produced `succeeded` plus an identity that `GET /v1/jobs/{id}/result` answers `unknown` about, for ever, with nothing logged as an error. Either record it in the data model beside `RunAbandonedError` and the schema-error case, with that reasoning, or decide the check does not belong in this milestone and remove it. What it must not stay is a terminal state a caller can receive and no artifact describes

## Phase 17: Convergence

- [X] T125 Complete the command block in `specs/009-asynchronous-runs/contracts/runs-layer.md` §CLI additions (partial). It shows `docdoc worker [--lease-seconds 90] [--max-attempts 3]` and `docdoc migrate [--check]` — three of the seven flags these two commands actually carry beyond the shared ones. Measured against `--help`: `worker` also takes `--run-database-url` and `--health-port`; `migrate` also takes `--run-database-url` and `--default-tenant`. **Two of the four omissions are this milestone's own additions**, which is what makes this worth fixing rather than shrugging at: `--health-port` is how FR-053's "both process types" is actually reached on a worker, and `--default-tenant` is the explicit step FR-089 requires — the one that records who owns the unprefixed store root and refuses to change its mind afterwards. A section headed "CLI additions" that omits two of the additions is describing a different command than the one shipped. Say what each does in one clause, and note that `--health-port` is off unless given, because a worker behind no load balancer has nobody to answer and binding a port nobody asked for is how a host network collides. The shared flags (`--json`, `--store`, `--schema-path`, the two limits, and the rest) stay out: they are on every subcommand and listing them here would make this block a duplicate of the global parser rather than a statement about these two commands
- [X] T126 Reconcile `tests/unit/test_run_state_machine.py` with the state machine it claims to hold (partial). Its docstring opens "The five states, their invariants, and the transitions that are refused", and `tasks.md`'s Phase 13 note records the same belief — "the 5 data-model invariants and 7 transition rules (all asserted in `test_run_state_machine.py`)". There are eight rules now, and three of them are asserted nowhere in that file: rule 3's abandonment (the word `abandon` does not appear in it), rule 2's substance now that `release` returns the attempt and requires ownership, and rule 8's `ResultNotStored`. **The coverage exists** — in `test_claim_policy.py`, `test_review_findings.py`, and `test_run_queue_postgres.py` — so no behaviour is untested and this is not a correctness gap. What is wrong is that the file the project designated as the statement of the machine is no longer that statement, and a reader who trusts its docstring gets an incomplete picture of which transitions are pinned. Two honest ways to close it and either is fine: move or duplicate the three assertions into this file so the docstring is true again, or narrow the docstring to what it does cover and name where the rest live. Prefer the first if the assertions are cheap against `InMemoryRunQueue`, which is the whole reason this file tests policy rather than SQL; prefer the second if moving them would need a database, because a rule only checkable with a container running is a rule most contributors never check
